export const arrRegiones = [
  ...
  {
    region: "Aisén del General Carlos Ibáñez del Campo",
    value: 14,
  },
  {
...
export const arrComunas = [
  ...
  {
    idComuna: 17,
    nombreComuna: "Ollague",
    idProvincia: 6,
  },
  {
    idComuna: 18,
    nombreComuna: "San Pedro de Atacama",
    idProvincia: 6,
  },
  {
    idComuna: 19,
    nombreComuna: "María Elena",
    idProvincia: 7,
  },
  {
    idComuna: 20,
    nombreComuna: "Tocopilla",
    idProvincia: 7,
  },
  {
    idComuna: 21,
    nombreComuna: "Chañaral",
    idProvincia: 8,
  },
  {
    idComuna: 22,
...
export const filtrarProvincias = (idRegion: number): { provincia: string; value: number }[] => {
  return arrProvincias
    .filter((provincia) => provincia.idRegion === idRegion)
    .reduce((acc, curr) => {
      acc.push({ provincia: curr.nombreProvincia, value: curr.idProvincia });

      return acc;
    }, [] as { provincia: string; value: number }[]);
};

export const filtrarComunas = (idProvincia: number): { comuna: string; value: number }[] => {
    return arrComunas
      .filter((comuna) => comuna.idProvincia === idProvincia)
      .reduce((acc, curr) => {
        acc.push({ comuna: curr.nombreComuna, value: curr.idComuna });
  
        return acc;
      }, [] as { comuna: string; value: number }[]);
  };
